...
### ProPresenter repertoire importer · 17 September

- [x] Phase 1 implemented: editor-only Song Bank import screen; SDK54 DocumentPicker/FileSystem; multiple selection with cache copies, strict UTF-8/BOM UTF-16 decoding, 50-file/100KiB-file/2MiB-batch limits and binary/blank rejection. Preserve headings, line breaks and repeated lyrics. Review/edit/skip in memory; permission note and explicit confirmation; no preview writes or original-file uploads. Delete only importer-owned DocumentPicker cache files, never source files. Lazy native imports keep existing development clients bootable until rebuilt.
- [x] Phase 2 implemented: authenticated create-only atomic `import_library_songs`, permission recheck, immutable batch UUID/payload receipt and title+artist duplicate checks. Lightweight paginated title/artist reads avoid downloading catalogue lyrics. Church advisory locks also guard direct library identity changes; existing duplicates, references and service snapshots are not rewritten. Record filename/type/permission with canonical attribution; default 4/4, no invented key/BPM. Apply migrations021/022 (trusted crypto search path) to the connected Supabase project.
- [x] Supabase import rollback fixtures and full weekly/WhatsApp fixtures pass: roles/church/prototype boundaries, kicked callers, private receipts, duplicate normalization/different artists, unchanged existing data, invalid-batch rollback, retry/altered-payload rejection and snapshot independence. Parallel authenticated-role SQL transactions produce one creation/one duplicate skip; concurrent identical-batch retries return the same result with one canonical row. Exact-owned disposable test church/account and their two songs/three receipts were removed; verified no remaining fixture church/account. No real songs or prototype data were removed.
- [x] TypeScript and all64 tests pass, including decoder/limits/cleanup, local-only review, permissions, frozen identical retry, duplicate skip and manifest guards. Final Android JavaScript export passes:1021 modules,2.02MB, no bytecode. Native source/mock tests do not establish phone behavior. Published CI is recorded separately after GitHub rollout.
- [x] Publish importer phases1/2 as [56d580d](https://github.com/HezkiMaarende/pSalmo/commit/56d580d2428b4e1e6ef5d0cf52a07accfbe781fb), verify exact tested tree `884c2578f3c11dfff925c882f60cc56554bc4bc2` and clean local/origin refs. [CI35209706807](https://github.com/HezkiMaarende/pSalmo/actions/runs/35209706807) succeeds: locked install, TypeScript,64 tests, Android JS export. Prior WhatsApp-review [CI35205658184](https://github.com/HezkiMaarende/pSalmo/actions/runs/35205658184) also succeeds. No device is currently attached; installation/phone trial remains open.
- [x] Fresh arm64 development APK compiled successfully in3m52s (396 tasks) with picker14.0.8/filesystem19.0.24. Block filesystem's legacy READ/WRITE_EXTERNAL_STORAGE plus MANAGE_EXTERNAL_STORAGE in Expo config; add pre-publication APK permission guard. Manifest-only incremental Ninja check hit a cached-header Windows260-character path limit; repackage with the20 byte-identical already-compiled native libraries, then verify signature and actual APK has no broad storage permissions. Final ignored artifact:68,668,414 bytes, SHA256 `0a9e51966c8c1acb2fc2d2b1439cf5da3c610b8f84906ab94c69a79f6d1af51c`. Development APK needs Metro; not a standalone WhatsApp build, not installed/device-tested here. Earlier fresh-appearance compile gate is now superseded by this native build; runtime/audio acceptance remains open.
- [x] Supabase Security Advisor rerun:0 error findings. Authenticated-only SECURITY DEFINER import is intentional and role-tested; private receipt RLS has no client policies deliberately. Existing leaked-password protection warning remains open. [RPC notice](https://supabase.com/docs/guides/database/database-linter?lint=0029_authenticated_security_definer_function_executable), [private receipt notice](https://supabase.com/docs/guides/database/database-linter?lint=0008_rls_enabled_no_policy), [password protection](https://supabase.com/docs/guides/auth/password-security#password-strength-and-leaked-password-protection).
- [ ] Dependency security follow-up: fresh online npm audit reports48 affected dependency nodes (16 moderate/32 high), rooted in transitive image-size/PostCSS/uuid advisories and propagated through the SDK54 tooling graph. No forced SDK upgrade/automatic audit rewrite was applied. Review compatible remediation and actual reachability before wider rollout; TXT importer does not parse images or CSS. [image parser advisory](https://github.com/advisories/GHSA-w3rx-r6r6-pgpr), [PostCSS advisory](https://github.com/advisories/GHSA-6g55-p6wh-862q), [uuid advisory](https://github.com/advisories/GHSA-w5hq-g745-h8pq).
- [ ] Phase 3 population: obtain user-permitted one-song-per-TXT exports; trial5–10 actual church songs, compare lyrics/credits/defaults with sources, then continue batches. No church repertoire was supplied/imported or invented. Permission is user-confirmed, not independently audited.
- [ ] Device acceptance: rebuilt native picker, cancellation, keyboard/large fonts, both themes, explicit confirmation, uncertain-response identical retry and imported-song-to-service flow. No picker/device success claimed. No broad storage permission is permitted; native manifests/APKs are checked before publication.
- [ ] Metronome/Hermes/timing/IEM/background testing remains deferred and unchecked below. This importer does not change click scheduling, service access or audio routing.
...
- [x] Phase 3: Metronome and its AddSongs are root-stack routes outside the tabs; regular AddSongs stays in each tab stack. Navigation chrome hides the global mini-player throughout both root routes. Setlist exposes global Stop even while saving/pending; Practice keeps its playback dock. Root Back has a native-compatible unsaved/busy removal guard and does not dispose the player. Editor entry stops playback. TypeScript and navigation/transport contracts pass; real Android multi-tab Back/keyboard acceptance remains a device gate.
...
- [x] Rollout review added Stop to hidden-player loading/error/empty/Add states and guarded unsaved manual Add drafts. Appearance/status-bar colors align with themed headers. No click algorithm, membership/service policy or Automator/Tracker change.
- [x] Phase4 CI [35190594718](https://github.com/HezkiMaarende/pSalmo/actions/runs/35190594718) successful.
- [x] Phase5 regression CI [35191132421](https://github.com/HezkiMaarende/pSalmo/actions/runs/35191132421) successful: locked install, TypeScript,44 tests and Android JS export. Final loading/status-bar adjustment was rerun locally with the same checks; native/device gates remain open.
...
1. **Populate Song Bank first (user-selected):** reviewed ProPresenter TXT importer implemented above. Trial5–10 permission-confirmed church exports on the phone, verify attribution/lyrics/defaults, then import remaining repertoire in batches. Keep metronome acceptance deferred on its open checklist. AI provider/consent evaluation follows catalogue curation, not guessed lyrics or automatic canonical promotion.
