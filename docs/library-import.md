...
1. Rebuild/install the native development app for the new SDK54 DocumentPicker module (`npm run build:android`); restart Metro using your established development workflow. Older binaries can still open other pages, but importing will report a missing native module. Use `build:share` separately if preparing a standalone APK for remote testers.
