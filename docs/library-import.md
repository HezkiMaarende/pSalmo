...
- Imported songs record `propresenter_text` or `propresenter_native`, source filename and permission basis. Canonical edits keep provenance. Adding them to a service uses the existing snapshot workflow; later canonical edits do not alter that service.
