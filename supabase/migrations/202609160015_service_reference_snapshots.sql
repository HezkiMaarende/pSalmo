alter table public.setlist_items add column library_references jsonb not null default '[]'::jsonb
  check(jsonb_typeof(library_references)='array');
create or replace function public.append_service_songs(target_service_id uuid,proposals text[] default '{}',canonical_song_id uuid default null)
returns void language plpgsql security invoker set search_path=public as $$
declare service public.services%rowtype; library_song public.songs%rowtype; list_id uuid;
  next_position integer; title text; reference_url text; reference_records jsonb;
begin
  ...
  if canonical_song_id is not null then
    ...
    select jsonb_agg(jsonb_build_object('label',label,'url',url,'position',position) order by position)
      into reference_records from public.song_references where song_id=library_song.id;
    insert into public.setlist_items(setlist_id,song_id,proposed_title,artist,position,key,bpm,time_signature,lyrics_or_chords,arrangement_url,library_references)
      values(list_id,library_song.id,library_song.title,library_song.artist,next_position,library_song.default_key,
        library_song.default_bpm,library_song.default_time_signature,library_song.lyrics,reference_url,coalesce(reference_records,'[]'::jsonb));
  else
