...
begin
  ...
  for entry in select value from jsonb_array_elements(reviewed_entries) loop
    ...
    if existing_id is not null then
      ...
      update public.songs set source_type=case when (entry->>'source_filename') ~* '\.txt$' then 'propresenter_text' else 'propresenter_native' end,source_filename=entry->>'source_filename',permission_basis=trim(permission_note) where id=song_id;
