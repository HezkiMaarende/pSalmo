...
test("Setlist settings send one atomic snapshot update, never a canonical song write", async () => {
  ...
  const chain = {
    ...
    async single() {
      return { error: denied ? { message: "Access denied" } : null };
    },
  };
  const module = { exports: {} };
  vm.runInNewContext(
    ts.transpileModule(
      fs.readFileSync(require.resolve("../src/lib/church.ts"), "utf8"),
      {
        compilerOptions: {
      ...
      require(name) {
        if (name === "./supabase")
          return {
            supabase: {
              from(table) {
                assert.equal(table, "setlist_items");
                return chain;
              },
            },
          };
  ...
  await assert.rejects(
    module.exports.saveSetlistSettings("item", { ...input, title: " " }),
    /wajib/,
  );
  assert.equal(updates.length, 1);
  denied = true;
  await assert.rejects(
    module.exports.saveSetlistSettings("item", input),
    /Access denied/,
  );
});
